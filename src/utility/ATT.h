...
public:
    ...
    virtual int attemptPairing(uint16_t handle);
    ...
    uint8_t localIRK[16] = {0x54, 0x83, 0x63, 0x7c, 0xc5, 0x1e, 0xf7, 0xec, 0x32, 0xdd, 0xad, 0x51, 0x89, 0x4b, 0x9e,
                            0x07};
...
private:
    ...
    virtual int
    findInfoReq(uint16_t connectionHandle, uint16_t startHandle, uint16_t endHandle, uint8_t responseBuffer[]);
    ...
    virtual void
    readOrReadBlobReq(uint16_t connectionHandle, uint16_t mtu, uint8_t opcode, uint8_t dlen, uint8_t data[]);
...
private:
    ...
    struct {
        ...
        uint8_t encryption;
        uint8_t IOCap[3];
        bool isPairingInitiator;
        uint8_t peerPairingConfirmValue[16];
