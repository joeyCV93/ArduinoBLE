...
int HCIClass::leReadPeerResolvableAddress(uint8_t peerAddressType, uint8_t *peerIdentityAddress,
                                          uint8_t *peerResolvableAddress) {
...
void HCIClass::handleAclDataPkt(uint8_t /*plen*/, uint8_t pdata[]) {
    ...
    if (aclHdr->cid == ATT_CID) {
        ...
        if (aclFlags == 0x1) {
            ...
            L2CAPSignaling.handleSecurityData(aclHdr->handle & 0x0fff, aclHdr->len,
                                              &_recvBuffer[1 + sizeof(HCIACLHdr)]);
        }
...
void HCIClass::handleEventPkt(uint8_t /*plen*/, uint8_t pdata[]) {
    ...
    if (eventHdr->evt == EVT_DISCONN_COMPLETE) {
    ...
    } else if (eventHdr->evt == EVT_ERROR) {
#ifdef _BLE_TRACE_
        ...
        switch ((LE_META_EVENT) leMetaHeader->subevent) {
            case 0x0A: {
                ...
                } *leConnectionComplete = (EvtLeConnectionComplete *) &pdata[sizeof(HCIEventHdr) +
                                                                             sizeof(LeMetaEventHeader)];
            ...
            case ADVERTISING_REPORT: {
                struct __attribute__ ((packed)) EvtLeAdvertisingReport {
                    ...
                    uint8_t eirLength;
                    uint8_t eirData[31];
                } *leAdvertisingReport = (EvtLeAdvertisingReport *) &pdata[sizeof(HCIEventHdr) +
                                                                           sizeof(LeMetaEventHeader)];
            ...
            case LONG_TERM_KEY_REQUEST: {
                ...
                if (foundLTK) {
                    ...
                    int result = sendCommand(OGF_LE_CTL << 10 | LE_COMMAND::LONG_TERM_KEY_REPLY, sizeof(ltkReply),
                                             &ltkReply);
...
#endif
                    sendCommand(OGF_LE_CTL << 10 | LE_COMMAND::LONG_TERM_KEY_NEGATIVE_REPLY, 2,
                                &ltkRequest->connectionHandle);
                }
                break;
            }
...
#if defined(_BLE_TRACE_) || defined(_BLE_PAIRING_TRACE_)
                    Serial.println("Set Peer Encryption to SENT_PUBKEY");
#endif

//If the peer was the initiator of the pairing, then this public key was sent in response to one already,
// otherwise the key was sent first so we want to handle in the response event handler and not do any of this yet.
            ...
            case GENERATE_DH_KEY_COMPLETE: {
                ...
                if (evtLeDHKeyComplete->status == 0x0) {
                    ...
                    btct.printBytes(DHKey,32);
                        ...
                        Serial.println("DHK Generated in HCI Layer");
#endif
...
//PAIRING STAGE 1B
bool HCIClass::attemptPairing(uint16_t handle) {
#ifdef _BLE_PAIRING_TRACE_
    Serial.print("Sending Pairing Request to L2CAPSignaling Layer");
    Serial.println(handle);
#endif

    bool messageSent = L2CAPSignaling.initiatePairingRequest(handle);
